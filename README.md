...
async def rock_paper_scissors(ctx, amount: str = None, choice: str = None):
    ...
    if result == "win":
        balance += value
        result_msg = f"🎉 You won! Your balance increased by {value:,}$."
    elif result == "lose":
        balance -= value
        result_msg = f"💀 You lost! Your balance decreased by {value:,}$."
    else:
        result_msg = f"🤝 It's a tie! Your balance remains the same."

    # Update gambled amounts (only if not a tie)
    if result != "tie":
        gambled += value
        total_gambled += value
    ...
    embed.add_field(name="Bot Choice", value=f"**{bot_choice.capitalize()}**", inline=True)
